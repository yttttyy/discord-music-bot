...
- **Search scoring**: every text search (`searchYouTube`) fetches 8 flat candidates and picks the best by score — "- Topic" channels boosted, tracks over 7 min (`MAX_SEARCH_DURATION`) penalized, content junk (летсплей/стрим/подкаст/…, `CONTENT_JUNK`) and non-original music (covers/karaoke/…, `MUSIC_JUNK`) penalized, view count as tiebreak. Spotify-sourced tracks set `preferTopic: true` which raises the Topic bonus and junk penalty. No hard filtering — worst case the least-bad candidate plays.
